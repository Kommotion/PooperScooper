from cogs.utils.palworld_utils.palworld_rcon.source_rcon import SourceRcon
from cogs.utils.palworld_utils.util import check_for_process, kill_process
...
class PalworldUtil:
    def __init__(
        ...
        if self.operating_system == "windows":
            ...
            self.server_launch_args.append("-RCONPort=25575")
    ...
    async def log_and_broadcast(self, message: str, log_level: str = "info"):
        ...
        try:
            await self.rcon.send_command("Broadcast", [message])
        except OSError as e:
            logger.warning(
    ...
    async def save_server_state(self) -> bool:
        ...
        if response.strip() == SAVE_FINISHED_RESPONSE:
            ...
            await self.log_and_broadcast("Save game state failed!")
            return False
    ...
    async def restart_server(
        ...
        await self.log_and_broadcast("Restarting server now.")

        # Find and end server process.
        if await check_for_process(self.palworld_server_proc_name):
        ...
        if backup_server:
            await self.take_server_backup()
        else:
            logger.info("Skipping server backup.")

        # Launch server.
        await self.launch_server(update_server=check_for_server_updates)
