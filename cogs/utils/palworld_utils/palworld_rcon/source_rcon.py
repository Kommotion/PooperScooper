...
class SourceRcon:
    ...
    async def receive_all(self, sock: socket.socket, bytes_in: int = 4096) -> bytes:
        response = b""
        while True:
            try:
    ...
    async def check_auth_response(self, auth_response_packet: bytes) -> bool:
    ...
    async def auth_to_rcon(self, socket: socket.socket) -> bool:
        ...
        auth_response = await self.receive_all(socket)
        if await self.check_auth_response(auth_response):
    ...
    async def establish_connection(self, socket: socket.socket) -> bool:
        tries = 0
        max_tries = 5
    ...
    async def execute_command(self, socket: socket.socket, command: str) -> str:
        command_packet = await self.create_packet(command)
        socket.sendall(command_packet)

        response = await self.receive_all(socket)
    ...
    async def _send_command(self, command: str, args: list = []) -> str:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            if not await self.establish_connection(s):
                return "Failed to establish connection."

            if not await self.auth_to_rcon(s):
            ...
            return await self.execute_command(s, command)

    async def send_command(self, command: str, args: list = []) -> str:
        tries = 0
        max_tries = 5
        response = None

        while tries < max_tries:
            response = await self._send_command(command, args)
            if response == "Failed to establish connection.":
                logger.debug("Failed to establish connection. Retrying in 5 seconds")
